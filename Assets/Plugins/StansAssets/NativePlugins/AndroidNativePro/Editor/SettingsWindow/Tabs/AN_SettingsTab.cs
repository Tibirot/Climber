...
namespace SA.Android {
    public class AN_SettingsTab : SA_GUILayoutElement {
        ...
        GUIContent Info = new GUIContent("Info[?]:", "Full communication logs between Native plugin part");
        ...
        public override void OnGUI() {
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Environment Management"))) {
                ...
                using (new SA_GuiBeginHorizontal()) {
                    ...
                    bool click = m_jarResolverLink.DrawWithCalcSize();
                    if (click) {
            ...
            {
                EditorGUILayout.HelpBox("When plugin need to have a valid URI for and image, it can be saved " +
                                        "using the Internal or External storage. " +
                                        "In case saving attempt is failed, alternative option will be used. " +
                                        "You can define if Internal or External storage should be preferred option.",
                    MessageType.Info);
